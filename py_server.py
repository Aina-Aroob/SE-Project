...
def run_decision_making():
    def task():
        ...
        result_json = Decision_Making_Module(batEdge_json)
...
output_video_path = os.path.abspath("output_video.mp4")

tk.Button(video_controls_frame, text="Play Input Video", command=lambda: play_video(input_video_path), **btn_style).pack(pady=10)
tk.Button(video_controls_frame, text="Play Output Video", command=lambda: play_video(output_video_path), **btn_style).pack(pady=10)
